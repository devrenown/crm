            ...
            <div class="col-sm-12 col-lg-12 m-b-20">
                ...
                @endphp
                 <ul class="list-unstyled">
                    ...
                    <li>
                        <strong>Billing Address: </strong> {!! $invoice->billing_address !!}
                    </li>
                        ...
                        <div class="table-responsive no-border">
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr>
                                        <th>{{ __('Subtotal') }}:</th>
                                        <td class="text-end">{{ LocaleSettings('currency_symbol') }} {{ number_format($invoice->subtotal, 2) }}</td>
                                    </tr>
                                    <tr>
                                        <th>{{ __('Tax') }}: <span class="text-regular">({{ optional($invoice->tax)->percentage ?? 0 }}%)</span></th>
                                        <td class="text-end">{{ LocaleSettings('currency_symbol') }} {{ number_format($taxAmount, 2) }}</td>
                                    </tr>
                                    <tr>
                                        <th>{{ __('Discount') }}: <span class="text-regular">({{ $invoice->discount ?? 0 }}%)</span></th>
                                        <td class="text-end">{{ LocaleSettings('currency_symbol') }} {{ number_format($discountedAmount, 2) }}</td>
                                    </tr>
                                    <tr>
                                        <th>Total:</th>
                                        <td class="text-end text-primary"><h5>{{ LocaleSettings('currency_symbol') }} {{ number_format((int)$invoice->subtotal - (int)$discountedAmount + (int)$taxAmount, 2) }}</h5></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
