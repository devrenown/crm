...
Route::middleware(['auth'])->group(function () {
    ...
    Route::middleware(['onboarding', 'check_plan'])->group(function () {
        ...
        Route::get('/leaves', [LeaveIndexController::class, 'index'])
            ->name('leaves.index');

        Route::get('/leaves/create', [LeaveCreateController::class, 'create'])
            ->name('leaves.create');

        Route::post('/leaves', [LeaveCreateController::class, 'store'])
            ->name('leaves.store');

        Route::get('/leaves/{leave}', [LeaveViewController::class, 'show'])
            ->name('leaves.show');
            
        Route::get('/leaves/{leave}/document', 
            [LeaveViewController::class, 'viewDocument']
        )->name('leaves.document.view')
        ->middleware(['signed']);

        Route::get('/leaves/{leave}/edit', [LeaveViewController::class, 'edit'])
            ->name('leaves.edit');

        Route::put('/leaves/{leave}', [LeaveApprovalController::class, 'update'])
            ->name('leaves.update');

        Route::delete('/leaves/{leave}', [LeaveDeleteController::class, 'destroy'])
            ->name('leaves.destroy');

        Route::post('/leaves/{leave}/approve', [LeaveApprovalController::class, 'approve'])
            ->name('leaves.approve');

        Route::post('/leaves/{leave}/reject', [LeaveApprovalController::class, 'reject'])
            ->name('leaves.reject');
