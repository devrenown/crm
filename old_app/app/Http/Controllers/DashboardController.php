...
class DashboardController extends BaseController
{
    ...
    public function index()
    {
        ...
        $clients = User::where('type', UserType::CLIENT->value)->get();
        $thisMonthClients = User::where('type', UserType::CLIENT->value)->whereMonth('created_at', Carbon::today())->get();

        if(activeRole() === UserType::TL->value) {
            $employees = User::where(['type' => UserType::EMPLOYEE->value, 'reporting_manager' => auth()->id()])->get();
        }else {
            $employees = User::where('type', UserType::EMPLOYEE->value)->get();
        }
        ...
        //attendances
        if (activeRole() === UserType::TL->value) {
            $absentees = User::where(['type' => UserType::EMPLOYEE->value, 'reporting_manager' => auth()->id()])->whereDoesntHave('attendances', function($query){
                return $query->whereDay('created_at', Carbon::today())->take(1);
            })->get();
        }else {
            $absentees = User::where('type', UserType::EMPLOYEE->value)->whereDoesntHave('attendances', function($query){
                return $query->whereDay('created_at', Carbon::today())->take(1);
            })->get();
        }

        $query = User::where('type', UserType::EMPLOYEE->value)
        ...
        $baseQuery = User::join('employee_details', 'users.id', '=', 'employee_details.user_id')
            ->select('users.id', 'users.firstname', 'users.middlename', 'users.lastname', 'users.avatar', 'employee_details.user_id', 'employee_details.dob', 'employee_details.date_joined')
            ->with('employeeDetail');
