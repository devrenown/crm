...
class TenantService 
{
	...
	*/
	...
	private static function generateSubDomain(string $companyName): string
	{
	    ...
	    // Base domain
	    $domain = "{$slug}.dev.renownsystem.com";
	    ...
	    if ($count > 0) {
	        ...
	        $suffix = Tenant::where('domain', 'LIKE', "{$slug}%.dev.renownsystem.com")->count();
	        $domain = "{$slug}-" . ($suffix + 1) . ".dev.renownsystem.com";
	    }
    ...
    public static function timezone(int $tenantId): string
    {
        return DB::table('settings')
            ...
            ->where('name', 'timezone')
            ->value('payload') ?? config('app.timezone');
    }
