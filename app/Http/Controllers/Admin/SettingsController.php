...
class SettingsController extends Controller
{
    ...
    public function updateCompany(Request $request, CompanySettings $settings)
    {
        ...
        $settings->probation_period = $request->probation_period ?? $settings->probation_period;
        $settings->fax = $request->fax ?? $settings->fax;
        $settings->website_url = $request->website_url ?? $settings->website_url;

        $settings->term_conditions = $request->term_conditions ?? $settings->term_conditions;
        $settings->about = $request->about ?? $settings->about;
        $settings->save();
    ...
    public function updateTheme(Request $request, ThemeSettings $settings)
    {
        ...
        if ($request->hasFile('logo_light')) {
            $logo_light = self::upload($request->file('logo_light'), $path, $settings->logo_light ?? '');
        }
        if ($request->hasFile('logo_dark')) {
            $logo_dark = self::upload($request->file('logo_dark'), $path, $settings->logo_dark ?? '');
        }
        if ($request->hasFile('favicon')) {
            $favicon = self::upload($request->file('favicon'), $path, $settings->favicon ?? '');
        }
