<style>
    .birthday-admin-card {
        ...
        background: url("{{ asset('images/birthday-background.png')  }}");
                ...
                @foreach ($upcomingBirthdays as $index => $user)
                    ...
                    <div class="carousel-item {{ $index === 0 ? 'active' : '' }} mb-2">
                        <div class="birthday-admin-card text-center text-white p-4 position-relative">
                            ...
                            <div class="years">
                                <span class="text-birthday">{{ number_format($years) }} Years</span>
                            </div>

                            <div class="mt-3 text-birthday fw-semibold">
                                <p class="fs-5">{{ date('m-d', strtotime($user->dob)) == $today ? 'Today 🥳' :  date('d M', strtotime($user->dob)) }}</p>
