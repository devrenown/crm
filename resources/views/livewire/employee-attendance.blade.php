...
<div>
    ...
    {{ hm($totalMinutes) }}
                    ...
                    <div class="statistics">
                        <div class="row">
                        @if ($clockedIn && $timeStarted)
                            <div class="col-md-12 text-center">
                                <div class="stats-box">
                                    <p>{{ __('Started At') }}</p>
                                    <h6>{{ $timeStarted->format('h:i A') }}</h6>
                                </div>
                            </div>
                            @endif
        ...
        @php
    $dayPercent   = min(100, ($minutesToday / 480) * 100);
    $weekPercent  = min(100, ($minutesWeek / 2400) * 100);
    $monthPercent = min(100, ($minutesMonth / 9600) * 100);
@endphp
        ...
        <div class="card-body">
            ...
            <div class="stats-list">
                ...
                <div class="stats-info">
                    <p>{{ __('Today') }}
                        <strong><small>{{ hm($minutesToday) }}</small></strong>
                    </p>
                    <div class="progress">
                ...
                <div class="stats-info">
                    <p>{{ __('This Week') }}
                        <strong><small>{{ hm($minutesWeek) }}</small></strong>
                    </p>
                    <div class="progress">
                ...
                <div class="stats-info">
                    <p>{{ __('This Month') }}
                        <strong><small>{{ hm($minutesMonth) }}</small></strong>
                    </p>
                    <div class="progress">
...
@php
    ...
    foreach ($records as $r) {
        ...
        if (!$shiftName && !empty($r['shift_name'])) {
            $shiftName = $r['shift_name'];
        }
    }

    $punchIn  = collect($starts)->min();
    $punchOut = collect($ends)->max();
@endphp

<tr>
    <td>{{ $loop->iteration }}</td>
    <td>{{ format_date($date) }}</td>
    <td>{{ $punchIn?->format('d M h:i A') }}</td>
    <td>
        @if (!$punchOut && \Carbon\Carbon::parse($date)->lt(now()->startOfDay()))
            <span class="text-danger">Miss Out</span>
        @else
            {{ $punchOut?->format('d M h:i A') }}
        @endif
    </td>
    <td>{{ $shiftName ?? '-' }}</td>
    <td>{{ hm($minutes) }}</td>
