        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($budgets)
                ...
                ->addColumn('type', function($row){
                    ...
                    if ($type === 'category') {
                        $name = 'Category: ' . optional($row->category)->name;
                        // or: $name = 'Category: ' . ($row->category?->name ?? '');
                    }  

                    if ($type === 'project') {
                        $name = 'Project: ' . optional($row->project)->name;
                        // or: $name = 'Project: ' . ($row->project?->name ?? '');
                    }

                    return ucwords($name);
                })
                ->addColumn('amount', function($row){
                    return LocaleSettings('currency_symbol').($row->amount ?? 0);
                })
                ->addColumn('revenue', function($row){
                    return LocaleSettings('currency_symbol').($row->total_revenue ?? 0);
                })
                ->addColumn('expenses', function($row){
                    return LocaleSettings('currency_symbol').($row->total_expense ?? 0);
                })
                ->addColumn('taxes', function($row){
                    return LocaleSettings('currency_symbol').($row->taxes ?? 0);
                })
                ...
                ->addColumn('action',function ($row){
                    $id = $row->id;
                    return view('accounting::budgets.actions',compact('id'));
                })
                ->rawColumns(['action','attachment'])
                ->make(true);   // 👈 also add true
            }
