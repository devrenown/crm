            ...
            <div class="col-sm-12">
                ...
                <form action="{{ route('invoices.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div x-data="{
                        subtotalVal: 0,
                        discount: 0,
                        taxRate: 0,        // percentage (e.g. 18)
                        taxAmount: 0,      // calculated amount

                        setTax(percent) {
                            this.taxRate = parseFloat(percent || 0);
                            this.calculateTotal();
                        },
                        ...
                        calculateTotal() {
                            ...
                            $('table.repeater > tbody > tr').each(function () {
                                const total = parseFloat($(this).find('input.totalInput').val()) || 0;
                                subtotal += total;
                            });

                            this.subtotalVal = subtotal;
                            this.taxAmount = (this.subtotalVal * this.taxRate) / 100;
                        },

                        applyDiscount() {
                            const discountPercent = parseFloat(this.discount || 0);
                            return (this.subtotalVal * discountPercent) / 100;
                        },

                        get grandTotal() {
                            return (
                                parseFloat(this.subtotalVal) +
                                parseFloat(this.taxAmount) -
                                parseFloat(this.applyDiscount() || 0)
                            ).toFixed(2);
                        }
                    }">
                        <div class="row">
                                ...
                                <div class="input-block mb-3">
                                    ...
                                    <select class="form-control select" name="client">
                                        ...
                                        @foreach ($clients as $client)
                                            @if (activeRole() === \App\Enums\UserType::SUPERADMIN->value)
                                            <option value="{{ $client->id }}">{{ $client->fullname .' ' . '(' . @$client->userOrganization->name .')' }}</option>
                                            @else
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover table-white">
                                        <tbody>
                                            ...
                                            <tr>
                                                <td colspan="5" class="text-end taxTd">{{ __('Tax') }} (<span x-text="taxRate"></span>%)</td>
                                                <td class="text-end pe-4">
                                                    <input type="text" class="form-control text-end" :value="taxAmount.toFixed(2)" name="taxes_sum" id="taxes_sum" readonly>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td class="text-end pe-4">
                                                    <input class="form-control text-end" type="text" x-model="discount" @input="calculateTotal()" name="discount">
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td class="text-end tdata-width pe-4">
                                                    <input type="text" :value="grandTotal" class="form-control text-end" name="grand_total" readonly>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>                               
