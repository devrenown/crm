            ...
            <x-slot name="right">
                <div class="col-auto float-end ms-auto">
                    ...
                    <div class="view-icons">
                        <a href="{{ route('projects.index') }}" class="grid-view btn btn-link active"><i
                                class="fa fa-th"></i></a>
                ...
                @foreach ($projects as $project)
                    <div class="col-lg-4 col-sm-6 col-md-4 col-xl-3 d-flex">
                        <div class="card w-100">
                            <div class="card-body">
                                @activeAnyCan(['view-projects', 'edit-project', 'delete-project'])
                                    <div class="dropdown dropdown-action profile-action">
                                        <a href="#" class="action-icon dropdown-toggle" data-bs-toggle="dropdown"
                                            aria-expanded="false"><i class="material-icons">more_vert</i></a>
                                        <div class="dropdown-menu dropdown-menu-right">
                                            @activeCan('view-projects')
                                                <a class="dropdown-item" href="{{ route('projects.show', ['project' => \Crypt::encrypt($project->id)]) }}">
                                            ...
                                            @activeCan('edit-project')
                                                <a class="dropdown-item" href="javascript:void(0)"
                                                    data-url="{{ route('projects.edit', ['project' => ($project->id)]) }}"
                                            ...
                                            @activeCan('delete-project')
                                                <a class="dropdown-item deleteBtn"
                                                    data-route="{{ route('projects.destroy', $project->id) }}" data-title="Delete Project"
                                ...
                                <small class="block text-ellipsis m-b-15">
                                    <span class="text-xs">{{ $project->tasks->count() ?? 0 }}</span> <span
                                        class="text-muted">{{ __('Opened Tasks') }}</span>
                                    <span class="text-xs">{{ $project->tasks->count() ?? 0 }}</span> <span
                                        class="text-muted">{{ __('Tasks Completed') }}</span>
                                </small>
                                <p class="text-muted">
                                    {{$project->short_desc}}
                                </p>
                                ...
                                <div class="project-members m-b-15">
                                    ...
                                    <ul class="team-members">
                                        @if (!empty($project->leader_id))
                                            <li>
                                                <a href="@activeCan('show-Employeeprofile') {{ route('employees.show', ['employee' => \Crypt::encrypt($project->leader_id)]) }} @else # @endactiveCan"
                                                    data-bs-toggle="tooltip" title="{{ $project->leader->fullname }}">
                                                    <img src="{{ !empty($project->leader->avatar) ? uploadedAsset($project->leader->avatar, 'users') : asset('images/user.jpg') }}"
                                                        alt="{{ __('Avatar') }}">
                                                </a>
                                            </li>
                                        @endif
                                            ...
                                            @if (!empty($projectTeam) && $projectTeam->count() > 4)
                                                <li class="dropdown avatar-dropdown">
                                                    <a href="#" class="all-users dropdown-toggle" data-bs-toggle="dropdown"
                                                        aria-expanded="false">{{ !empty($projectTeam) && $projectTeam->count() > 0 ? ($projectTeam->count() - 4) : '' }}</a>
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <div class="avatar-group">
                                                            @foreach ($projectTeam as $member)
                                                                <a class="avatar avatar-xs" data-bs-toggle="tooltip"
                                                                    title="{{ $member->user->fullname }}"
                                                                    href="@activeCan('show-Employeeprofile') {{ route('employees.show', ['employee' => \Crypt::encrypt($member->user->id)]) }} @else # @endactiveCan">
                                                                    <img src="{{ !empty($member->user->avatar) ? uploadedAsset($member->user->avatar, 'users') : asset('images/user.jpg') }}"
